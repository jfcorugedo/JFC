...
import com.myhome.controller.RoleController;
...
privileged aspect RoleController_Roo_Controller_Json {
    ...
    @ResponseBody
    public ResponseEntity<String> RoleController.jsonFindRolesByNameEquals(@RequestParam("name") String name) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json; charset=utf-8");
        return new ResponseEntity<String>(Role.toJsonArray(Role.findRolesByNameEquals(name).getResultList()), headers, HttpStatus.OK);
    }
