...
import com.myhome.controller.UserController;
...
privileged aspect UserController_Roo_Controller_Json {
    ...
    @ResponseBody
    public ResponseEntity<String> UserController.jsonFindUsersByEmailEquals(@RequestParam("email") String email) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json; charset=utf-8");
        return new ResponseEntity<String>(User.toJsonArray(User.findUsersByEmailEquals(email).getResultList()), headers, HttpStatus.OK);
    }
